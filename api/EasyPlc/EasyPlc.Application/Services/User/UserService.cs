...
public class UserService : DbRepository<SysUser>, IUserService
{
    ...
    {
        await CheckPermission(input.Id, EasyPlcConst.Disable);//检查权限
    ...
    {
        await CheckPermission(input.Id, EasyPlcConst.Enable);//检查权限
    ...
    {
        await CheckPermission(input.Id, EasyPlcConst.ResestPwd);//检查权限
