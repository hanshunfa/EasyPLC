...
public class AuthService : IAuthService
{
    ...
    public async Task<LoginOutPut> Login(LoginInput input, LoginClientTypeEnum loginClientType)
    {
        ...
        {
            return await ExecLoginC(userInfo, input.Device, loginClientType);//执行c端登录
        }
    }
    ...
    /// <param name="loginClientType">登录类型</param>
    /// <returns></returns>
    public async Task<LoginOutPut> ExecLoginC(SysUser sysUser, AuthDeviceTypeEumu device, LoginClientTypeEnum loginClientType)
    {
        if (sysUser.UserStatus == DevDictConst.COMMON_STATUS_DISABLED) throw Oops.Bah("账号已停用");//账号冻结
                                                                                               //登录事件参数
        var logingEvent = new LoginEvent
        ...
        await WriteTokenToRedis(logingEvent, loginClientType);//写入token到redis
        await _eventPublisher.PublishAsync(EventSubscriberConst.LoginC, logingEvent);//发布登录事件总线
        //返回结果
        return new LoginOutPut
        {
            Token = string.Empty,
