...
public class SysOrgService : DbRepository<SysOrg>, ISysOrgService
{
    ...
    /// <inheritdoc />
    public async Task Add(SysOrgAddInput input, string name = EasyPlcConst.SysOrg)
    {
    ...
    public async Task Copy(SysOrgCopyInput input)
    {
        ...
        if (target != null || input.TargetId == EasyPlcConst.Zero)
        {
            ...
            {
                it.Names = it.ParentId == EasyPlcConst.Zero ? it.Name : GetNames(orgList, it.ParentId, it.Name);
            });
    ...
    /// <inheritdoc />
    public async Task Edit(SysOrgEditInput input, string name = EasyPlcConst.SysOrg)
    {
        await CheckInput(input, name);//检查参数
        var sysOrg = input.Adapt<SysOrg>();//实体转换
    ...
    /// <inheritdoc />
    public async Task Delete(List<BaseIdInput> input, string name = EasyPlcConst.SysOrg)
    {
        //获取所有ID
    ...
    public List<SysOrg> CopySysOrgChilden(List<SysOrg> orgList, long parentId, long newParentId)
    {
        ...
        if (orgs.Count > 0)//如果数量大于0
        {
            ...
            foreach (var item in orgs)//遍历组织
            {
                ...
                var childen = CopySysOrgChilden(orgList, id, item.Id);//获取子节点
                data.AddRange(childen);//添加子节点);
            }
            return data;//返回结果
        }
