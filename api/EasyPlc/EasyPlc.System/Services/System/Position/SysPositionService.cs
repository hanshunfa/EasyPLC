...
public class SysPositionService : DbRepository<SysPosition>, ISysPositionService
{
    ...
    /// <inheritdoc />
    public async Task Add(PositionAddInput input, string name = EasyPlcConst.SysPos)
    {
    ...
    /// <inheritdoc />
    public async Task Edit(PositionEditInput input, string name = EasyPlcConst.SysPos)
    {
        await CheckInput(input, name);//检查参数
        var sysPosition = input.Adapt<SysPosition>();//实体转换
    ...
    /// <inheritdoc />
    public async Task Delete(List<BaseIdInput> input, string name = EasyPlcConst.SysPos)
    {
        //获取所有ID
