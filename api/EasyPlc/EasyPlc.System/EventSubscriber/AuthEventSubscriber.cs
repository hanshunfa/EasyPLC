...
public class AuthEventSubscriber : IEventSubscriber, ISingleton
{
    ...
    public async Task LoginB(EventHandlerExecutingContext context)
    {
    ...
    /// <param name="context"></param>
    /// <returns></returns>
    [EventSubscribe(EventSubscriberConst.LoginC)]
    public async Task LoginC(EventHandlerExecutingContext context)
    {
        var loginEvent = (LoginEvent)context.Source.Payload;//获取参数
        var LoginAddress = GetLoginAddress(loginEvent.Ip);
        var sysUser = loginEvent.SysUser;
        ...
        _simpleCacheService.HashAdd(CacheConst.Cache_SysUser, sysUser.Id.ToString(), sysUser); //更新Redis信息
        LoginUser.UserId = sysUser.Id.ToString();
        LoginUser.Account = sysUser.Account;
        LoginUser.Name = sysUser.Name;
        LoginUser.OrgId = sysUser.OrgId.ToString();
        LoginUser.IsLogin = true;
        LoginUser.IsSuperAdmin = sysUser.Account == "superAdmin";
